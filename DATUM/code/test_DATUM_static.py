...
from data.dataset_video_train import DataLoaderTurbImageTest, DataLoaderTurbImageTestUg2
...
def get_args():
    ...
    parser.add_argument('--valid_frames', type=int, default=1, help='number of frames for evaluation')
...
def main():
    ...
    # test_dataset = DataLoaderTurbImageTestUg2(rgb_dir=args.val_path, num_frames=num_frames, total_frames=50, \
    #     im_size=args.patch_size, noise=0.0001, other_mod='tilt', start_frame=start_frame)
    test_dataset = DataLoaderTurbImageTestUg2(rgb_dir=args.val_path, num_frames=num_frames, total_frames=args.total_frames, \
        im_size=args.patch_size, noise=0.0001, start_frame=start_frame)
    ...
    with torch.no_grad():
        ...
        for s, data in enumerate(test_loader):
            ...
            seq_path = os.path.split(data[1][0])[-1]
            ...
            out_frames = test_tensor_img(output)
            ...
            for i in range(valid_frames):
                ...
                out_save.save(os.path.join(result_dir, f'{seq_path}_out.png'), "PNG")
                # out_save.save(os.path.join(out_folder, f'data_0005.png'), "PNG")
