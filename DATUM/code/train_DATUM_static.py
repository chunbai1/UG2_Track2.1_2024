...
def get_args():
    ...
    parser.add_argument('--num_frames', type=int, default=36, help='number of frames for the model')
    parser.add_argument('--num_workers', type=int, default=8, help='number of workers in dataloader')
    parser.add_argument('--train_path', type=str, default='/home/zhan3275/data/ATSyn_static/train_static', help='path of training imgs')
    ...
    parser.add_argument('--run_name', type=str, default='DATUM_static', help='name of this running')
    ...
    parser.add_argument('--train_flow', action='store_true', help='enable the training of optical flow, true will set spynet_path to None')
    ...
    parser.add_argument('--other_mod', type=str, default='blur')
...
def main():
    ...
    train_dataset = DataLoaderTurbImage(rgb_dir=args.train_path, num_frames=args.num_frames, total_frames=50, \
        im_size=args.patch_size, noise=0.0004, other_mod=args.other_mod,is_train=True)
    ...
    val_dataset = DataLoaderTurbImage(rgb_dir=args.val_path, num_frames=args.num_frames, total_frames=50, \
        im_size=args.patch_size, noise=0.0004, other_mod=args.other_mod, is_train=False)
    ...
    for epoch in range(1000000):
        for data in train_loader:
            ...
            if iter_count % 500 == 0:
                psnr_batch, ssim_batch = eval_tensor_imgs(target, output, input_, save_path=result_img_path, kw='train', iter_count=iter_count) # result_img_path
            else:
                psnr_batch, ssim_batch = eval_tensor_imgs(target, output, input_)
            ...
            if iter_count>0 and iter_count % args.val_period == 0:
                psnr, ssim, val_loss = validate(args, model, val_loader, criterion_char, iter_count, 200, result_img_path, device, 'static')
